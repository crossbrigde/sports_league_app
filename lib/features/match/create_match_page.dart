...
class _CreateMatchPageState extends State<CreateMatchPage> {
  ...
  int targetPoints = 0;
  int matchMinutes = 0;
  String tournamentType = 'regular'; // 改為字符串類型，預設為積分賽
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    RadioListTile<String>(
                      title: const Text('積分賽'),
                      subtitle: const Text('常規積分制比賽'),
                      value: 'regular',
                      groupValue: tournamentType,
                    ...
                    RadioListTile<String>(
                      ...
                      subtitle: const Text('敗者復活機會的賽制'), // 更新描述
  ...
  // 創建雙淘汰賽
  Future<void> _createDoubleEliminationTournament({
    required int numPlayers,
    int? targetPoints,
    int? matchMinutes,
    List<String>? playerNames,
    ...
    try {
      print('_createDoubleEliminationTournament - 檢查參數:');
      print('- numPlayers: $numPlayers');
      print('- targetPoints: $targetPoints');
      print('- matchMinutes: $matchMinutes');
      print('- playerNames: $playerNames');
      ...
      final tournament = await _bracketService.createDoubleEliminationTournament(
        name: tournamentName,
        numPlayers: numPlayers,
        targetPoints: targetPoints,
        matchMinutes: matchMinutes,
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('創建雙淘汰賽失敗: $e')),
        );
      }
    }
  }

  void _showDoubleEliminationDialog() {
    int dialogNumPlayers = numPlayers; // 預設值，可以從 numPlayers 獲取
    List<String> playerNames = [];
    bool randomPairing = false;
    int? targetPoints;
    int? matchMinutes;
              ...
              child: Column(
                ...
                children: [
                  ...
                  TextFormField(
                    ...
                    initialValue: dialogNumPlayers.toString(),
                    validator: (value) {
                      if (value == null || value.isEmpty) return '請輸入參賽人數';
                      final n = int.tryParse(value);
                      if (n == null) return '請輸入有效的數字';
                      if (n < 4 || n > 16) return '參賽人數必須在4到16人之間';
                      return null;
                    },
                    onChanged: (value) {
                      final newNumPlayers = int.tryParse(value) ?? 8;
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                // 在這裡可以加入表單驗證
                Navigator.pop(context); 
                await _createDoubleEliminationTournament(
                  numPlayers: dialogNumPlayers,
                  targetPoints: targetPoints,
                  matchMinutes: matchMinutes,
                  playerNames: playerNames,
